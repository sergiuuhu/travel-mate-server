...
export const generateDates = (weeks) => {
  ...
  currentDate.setDate(currentDate.getDate() - currentDate.getDay() + 3); // Set to Thursday of the current week

  const daysToAdd = weeks * 7;

  const dateFromThursday = new Date(currentDate);
  dateFromThursday.setDate(
    currentDate.getDate() + ((3 - currentDate.getDay() + 7) % 7) + daysToAdd,
  );

  const dateFromFriday = new Date(dateFromThursday);
  dateFromFriday.setDate(dateFromThursday.getDate() + 1);

  const dateFromSaturday = new Date(dateFromThursday);
  dateFromSaturday.setDate(dateFromThursday.getDate() + 2);

  const dateToSunday = new Date(dateFromThursday);
  dateToSunday.setDate(dateFromThursday.getDate() + 3);

  const date_to = formatDate(dateToSunday);

  const result = [
    { date_from: formatDate(dateFromThursday), date_to },
    { date_from: formatDate(dateFromFriday), date_to },
    { date_from: formatDate(dateFromSaturday), date_to },
  ];
...
export const letsGo = async () => {
  ...
  const dayStart = moment(dateTo, "DD/MM/YYYY")
    .subtract(1, "day")
    .toISOString();
